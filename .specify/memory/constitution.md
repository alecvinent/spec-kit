...
Bump rationale: Minor update to introduce the Spec-Driven AI Execution Protocol, 
  incorporating "Dreaming Phase" (Pre-Plan Simulation) and "Self-Reflective Inquiry" 
  (SRI) to enforce anti-vibe-coding and rigorous architectural analysis prior to 
  implementation.
...
Principles defined:
  ...
  VI.  Mandatory Pre-Plan Simulation (Dreaming Phase)
  VII. Self-Reflective Inquiry (SRI Protocol) & Anti-Vibe-Coding
...
Templates reviewed for alignment:
  ✅ .specify/templates/plan-template.md — generic "Constitution Check" gate (line 39)
       remains valid; gates are now concretely populated by Principles I–VII at plan time.
...
Before creating any technical architecture, plan, or set of tasks (`/speckit-plan` or `/speckit-tasks`), the AI agent MUST execute a **Dreaming Simulation Phase**.
...
Before writing or revising any artifact (`spec.md`, `plan.md`, or executable code via `/speckit-implement`), the agent MUST critique its own work and resolve gaps:
- **Self-Critique & Gap Identification:** Expose implicit assumptions, missing boundaries, and technical failure risks. Never invent silent fallbacks for missing specifications.
- **Inquiry Resolution:** Document gaps and resolve ambiguities in `.specify/sri_audit.md` before proceeding to task generation.
- **Traceability:** Architectural choices must include a "Failure Scenario Analysis" derived from the Dreaming report. No code shall be generated without an explicit prior SRI trace.

---

## SRI & Workflow Execution Matrix

When operating across Spec Kit skills, the AI agent must adhere to the following workflow extensions to ensure strict alignment with Principles VI and VII:
...
- **Authority.** Principles I–VII are binding gates. The `## Constitution Check` section of the
...
**Version**: 1.1.0 | **Ratified**: 2026-06-19 | **Last Amended**: 2026-09-16
